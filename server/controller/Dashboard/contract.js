  ...
  async (req, res) => {
    try {
      ...
      contractRequest.recipient.map((val, i) => {
        ...
        const sentEmail = contactDetailsList
        ...
        `;
          ...
          sendMail(
            email,
            "Assinatura de Contrato - Vanth Docs System",
            htmlContent,
          );
        });
      });
...
router.post("/get-contract-details", async (req, res) => {
  ...
  const { companyId, contractId, docusignEnvelopeId, recipientViwer } =
    req.body;
  ...
  if (contractRequest.ableToSign != recipientViwer) {
    return res.json({
      data: {
        ...
        ableRecipient: contractRequest.recipient.filter(item => {
          if (item.id == contractRequest.ableToSign) {
            return item;
          }
        }),
        success: false,
  ...
  if (
    ...
    if (matchingRecipient) {
      ...
      console.log(
        "Erro: Nenhum contato correspondente encontrado para o recipientViwer fornecido.",
      );
    }
    ...
    const token = await docusign.getDocuSignJwtToken();
    const results = await docusign.generateRecipientViewRequest(
      token,
      args.envelopeId,
      view,
    );
...
router.post("/update-contract-status", async (req, res) => {
  ...
  if (contractRequest) {
    console.log(
      "Started working on downloading & storing the document on the cloud",
    );
    ...
    for (let i = 0; i < contractRequest.contractDocumentIds.length; i++) {
      ...
      const fileNameValue = originalFileName
        .split(" ")
        .join("_")
        .split(".")
        .filter((item, index) => index < fileNameArray.length - 1)
        .join("_");
