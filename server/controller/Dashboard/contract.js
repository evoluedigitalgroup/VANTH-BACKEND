...
router.post("/filter-contracts", authentication.UserAuthValidateMiddleware, async (req, res) => {
  ...
  if (req.user.permissions.contract) {
    ...
    const findDataWithSignedUrl = await Promise.all(findData.map(async (item, index) => {
      try {
        const signedFileUrl = await getContractDetail(item.signatureEnvelopeId);
        return {
          ...item._doc,
    ...
    const countItemsWithSignedUrl = findDataWithSignedUrl.reduce((acc, item) => {
    ...
    res.json({
      success: true,
      data: { findData: findDataWithSignedUrl, totalFindData, countItemsWithSignedUrl },
...
router.post("/create-contract", authentication.UserAuthValidateMiddleware, async (req, res) => {
  try {
    ...
    const zapSignResponse = await axios.post(config.zapsignUrl + 'docs/', zapSignPayload, {
      headers: {
...
router.post("/get-contract-details", async (req, res) => {
  ...
  if (contractRequest.status === 'signed' || contractRequest.status === 'rejected') {
    ...
    const results = await getContractDetail(contractRequest.signatureEnvelopeId);
...
router.post("/update-contract-status", async (req, res) => {
  ...
  if (contractRequest) {
    ...
    for (let i = 0; i < contractRequest.contractDocumentIds.length; i++) {
      ...
      const signedFileUrl = await getContractDetail(contractRequest.signatureEnvelopeId);
...
router.post("/update-contract-approval-status", authentication.UserAuthValidateMiddleware, async (req, res) => {
  ...
  const mensagem = 'O contrato foi aceito com sucesso. A assinatura foi validada pelo sistema Vanth Docs. Obrigado por confiar em nossos serviços.';
  if (updateStatus.contractDocumentIds[0].isApproved === 'approved') {
    try {
      await sendMail(signerEmail, 'Status de Aprovação do Contrato - Vanth Docs System', htmlAprove);
    ...
    try {
      await sendMail(signerEmail, 'Status de Aprovação do Contrato - Vanth Docs System', htmlReject);
      await twilioClientSenderSMS(mensagem, signerPhone);
      await twilioClientSenderWhatsApp(mensagem, signerPhone);
    } catch (err) {
      console.log(err);
